...
``0001`` 至 ``0075`` 的首轮多语言 RST 已完成。``0001`` 至 ``0050`` 的逐题规则提炼已经完成，
其稳定结论正在应用于 ``0051`` 至 ``0100``。

当前批次完成：

* ``0074. Search a 2D Matrix``：全局有序矩阵、扁平坐标映射与闭区间二分；
* ``0075. Sort Colors``：荷兰国旗三向分区、未知区间与交换后复查。

下一内容批次从 ``0076`` 开始，按难度预算处理 ``0076. Minimum Window Substring`` 和
``0077. Combinations``。``0078. Subsets`` 留到后续批次。
