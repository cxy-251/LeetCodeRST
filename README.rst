...
下一题是 ``0041. First Missing Positive``。实际进度以 ``state/PROGRESS.toml`` 为准。
...
0001 至 0040 已完成。当前样板已经覆盖哈希表、链表、滑动窗口、二分分割、回文
...
数独约束验证、位掩码回溯求解、游程编码、可重复组合枚举与同层去重组合枚举。
下一题从 0041 开始；自动任务按难度预算连续精写 1 至 3 道题，并对每道题独立执行完整质量门。
