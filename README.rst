...
``0161`` 与 ``0163`` 已按 Premium 规则登记。``0001`` 至 ``0100`` 的两轮历史逐题审查、81 条规则归并和
14 项事实更正也已完成。

本次返工已把 ``0129–0165`` 范围内的 31 道公开题恢复为“每题唯一自包含 RST”，十语言代码全部内联，
并删除 169 个已消费的 ``.inc`` 分片。题目算法、完成题数、Premium 状态和下一题均未改变。

下一批为 ``0166–0169``：Medium、Medium、Easy、Easy，合计 6 分。从 ``0151`` 起，所有后续题目统一使用
并持续维护 ``docs/FORWARD_RULES_0151_ONWARD.rst``。

固定“每 50 题一次”的全量审核已经取消。普通批次执行自身质量门；只有出现高风险语义、验证异常、规则冲突，
或用户明确要求时，才进行针对性抽查。
...
* ``archive/reviews/0051-0100/MANIFEST.rst``：第二次逐题审查与归并产物；
...
``state/CONCEPT_LEDGER.toml`` 使用分段记录，后续分段中的同名 ``updates`` 覆盖更早记录的最新题号、
出现次数、教学状态和说明。
...
仓库任务禁止创建分支、PR、上传分片分支或临时组装工作流。多文件提交使用 Git 对象直接构造，不经过任何
旁路分支。
