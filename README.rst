...
``0001`` 至 ``0152`` 的多语言教程已经完成。``0001`` 至 ``0100`` 的两轮历史逐题审查、81 条规则归并和
14 项事实更正也已完成。

``0150`` 至 ``0152`` 按难度预算 6 分完成，三题均为 Medium。下一批是 ``0153–0154``，由 Medium 和
Hard 组成，合计 6 分；``0155`` 为 Medium，加入后会超过单批预算。从 ``0151`` 起，所有后续题目统一使用
并持续维护 ``docs/FORWARD_RULES_0151_ONWARD.rst``，不再按题号区间新建规则文件。
