...
``0001`` 至 ``0140`` 的多语言教程已经完成。``0001`` 至 ``0100`` 的两轮历史逐题审查、81 条规则归并和
14 项事实更正也已完成。

``0139`` 至 ``0140`` 按难度预算 6 分完成：Medium、Hard。下一批是 ``0141–0144``，
由 Easy、Medium、Medium、Easy 组成，合计 6 分；``0145`` 为 Easy，加入后会超过单批预算。新题继续直接
执行 ``docs/FORWARD_RULES_0101_0150.rst``。
