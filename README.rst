...
当前已经完整审查 ``0001`` 至 ``0035``，下一批为 ``0036`` 至 ``0040``。
...
* 字符串题分别定义比较单位和返回下标坐标；
* 主代码必须包含正文确认的必要守卫、导入和修复，不能把已知补丁留在代码块之后；
* 无边界递减、``pop``、``unwrap`` 和非空断言必须由紧邻的见证元素或哨兵不变量支撑；
* ``size_t``、``usize`` 和容器长度窄化到 ``int`` / ``i32`` 前，精确上界必须覆盖边界位置 ``n``；
* 二分函数必须区分命中下标、插入边界、末尾外位置和失败哨兵；
* 原地排列证明元素多重集保持、结果方向、邻接最优和回绕分支；
* 复杂度计入输入规范化、键载荷、字符工作、具体容器、输出构造、结果扩容和返回复制；
* C 失败路径不得返回部分结果，分配结果必须在首次写入前检查；
* ``realloc`` 与复合状态分配必须事务式更新；
