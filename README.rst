...
``0001`` 至 ``0104`` 的多语言教程已经完成。``0051`` 至 ``0100`` 的第二轮逐题审查、81 条规则归并和
14 项事实更正也已完成。

``0101`` 至 ``0104`` 按 Easy=1、Medium=2、Hard=4 的难度预算完成，总分为 6。下一批是
``0105–0107``，三道 Medium 合计 6 分。新题继续执行 ``docs/FORWARD_RULES_0101_0150.rst``。

核心入口
--------

* ``AGENTS.md``：新对话接手顺序、难度预算和逐题闭合工作流；
...
* ``problems/0101-0200/README.rst``：当前范围题目索引和批次预算。
...
``state/CONCEPT_LEDGER.toml`` 使用分段记录，后续分段中的同名 ``updates`` 覆盖更早记录的最新题号、出现
次数、教学状态和说明。
