...
``0001`` 至 ``0086`` 的首轮多语言 RST 已完成。``0001`` 至 ``0050`` 的逐题规则提炼已经完成，
其稳定结论正在应用于 ``0051`` 至 ``0100``。

当前批次完成：

* ``0085. Maximal Rectangle``：按行柱高压缩、柱状图复用与底边归属证明；
* ``0086. Partition List``：稳定双链、逐节点断开与节点守恒。

下一内容批次从 ``0087`` 开始，按难度预算处理 ``0087. Scramble String`` 和
