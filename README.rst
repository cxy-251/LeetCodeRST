...
``0001`` 至 ``0088`` 的首轮多语言 RST 已完成。``0001`` 至 ``0050`` 的逐题规则提炼已经完成，
其稳定结论正在应用于 ``0051`` 至 ``0100``。

当前批次完成：

* ``0087. Scramble String``：三维区间状态、交换转移、字符频次剪枝与假状态缓存；
* ``0088. Merge Sorted Array``：逆向双指针、未读前缀保护与原地有序合并。

下一内容批次从 ``0089`` 开始，按难度预算处理 ``0089. Gray Code``、``0090. Subsets II`` 和
``0091. Decode Ways``。
