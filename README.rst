...
``0001`` 至 ``0061`` 的首轮多语言 RST 已完成。``0001`` 至 ``0050`` 的逐题规则提炼已经完成，
其稳定结论正在应用于 ``0051`` 至 ``0100``。

当前批次完成：

* ``0060. Permutation Sequence``：阶乘分块、零基秩与候选删除；
* ``0061. Rotate List``：有效旋转量、临时成环与切点断环。

下一内容批次从 ``0062`` 开始，按难度预算处理 ``0062. Unique Paths``、
``0063. Unique Paths II`` 和 ``0064. Minimum Path Sum``。
