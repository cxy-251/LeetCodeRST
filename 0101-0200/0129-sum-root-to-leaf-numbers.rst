...
:教学重点: 前缀乘十加位、叶节点结算、只读遍历、数值宽度

题目重述
--------

二叉树每个节点保存 ``0..9`` 的数字。每条根到叶路径按顺序组成一个十进制整数，返回所有这些整数之和。前导零不改变数值；叶节点必须同时没有左右孩子。
...
每个节点访问一次，时间 ``O(n)``。递归栈 ``O(h)``；显式栈最坏 ``O(n)``。路径数组基准还需 ``O(h)`` 缓冲，并在每个叶节点重新遍历路径。固定宽语言使用 64 位中间状态。
