...
这等价于把每一段连续上涨区间在起点买入、在终点卖出；一段连续上涨也可以拆成若干相邻交易，
利润仍然等于这段的全部正相邻差之和。关键是按“上涨段”计数，而不是把共享日期的每条上涨边都当成一笔独立交易。
...
**可达性。** 把相邻正差合并成最大连续上涨段；每段在第一天买入、最后一天卖出，
得到该段所有正相邻差之和。每段至少消耗两个日期，且不同上涨段之间由下降或平稳日期隔开，
所以这样的交易段数量不超过 ``floor(n/2)``。当 ``k >= floor(n/2)`` 时，额度足以完成全部这些交易，
并且不会同时持有多股。
