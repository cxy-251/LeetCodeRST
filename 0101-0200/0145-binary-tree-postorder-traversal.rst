...
:重点: 左右根顺序、根右左中间序列、单栈状态

题目重述
--------

给定二叉树的根节点 ``root``，返回它的后序遍历结果。对每棵非空子树依次遍历左子树、右子树，最后访问根节点；空树返回空数组，值相同的不同节点仍分别输出。
...
后序目标是 ``左 + 右 + 根``。对整个序列反转后得到 ``根 + reverse(右) + reverse(左)``；按同样规则递归到每棵子树，直接构造根、右、左并整体反转即可。
