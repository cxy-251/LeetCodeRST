...
:重点: 单词顺序反转、首尾空格删除、连续空格压缩

题目重述
--------

给定字符串 ``s``，返回其中所有单词按出现顺序完全反转后组成的新字符串。单词是由连续非空格字符构成的最大片段；输入中可能存在前导空格、尾随空格以及多个连续空格。

输出只保留原有单词，每两个相邻单词之间恰好放置一个空格，且字符串首尾不能有空格。``s`` 的长度在 ``1..10^4`` 范围内，只包含英文字母、数字和空格，并且至少包含一个单词。
...
.. code-block:: text
   ...
   输入：s = "   code   reads  clearly "
   输出："clearly reads code"
   解释：三个单词的顺序反转，同时删除首尾空格并把连续空格压缩为一个。

.. code-block:: text

   输入：s = "one"
   输出："one"
   解释：只有一个单词时顺序不变，输出中也没有额外空格。
