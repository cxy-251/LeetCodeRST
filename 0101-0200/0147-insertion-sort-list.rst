...
:重点: 插入排序过程、有序前缀、节点重连、非递减结果

题目重述
--------

给定单链表头节点 ``head``，使用插入排序把链表按节点值从小到大排列，并返回排序后的头节点。插入排序从第二个节点开始，依次把当前节点插入前面已经排好序的链表片段中，直到所有节点都被处理。

链表节点数在 ``1..5000`` 范围内，节点值在 ``-5000..5000`` 范围内。排序结果必须包含原链表中的全部节点，并按非递减顺序连接。
...
.. code-block:: text
   ...
   输入：head = [6,1,4,1,3]
   输出：[1,1,3,4,6]
   解释：两个值为 1 的节点都会保留，所有原节点按非递减顺序重新连接。

.. code-block:: text

   输入：head = [-2,5,0,-4]
   输出：[-4,-2,0,5]
   解释：插入排序逐个把节点放入前面的有序部分，最终最小值 -4 位于链表头部。
