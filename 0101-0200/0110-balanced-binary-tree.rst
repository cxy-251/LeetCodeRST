...
给定二叉树根节点 ``root``，判断整棵树是否高度平衡。高度平衡要求树中的每个节点都满足：其左子树与右子树的高度差绝对值不超过 ``1``。该条件必须对所有后代节点成立；空树视为高度平衡。

树中节点数在 ``0..5000`` 范围内，节点值在 ``-10^4..10^4`` 范围内。
...
.. code-block:: text
   ...
   输入：root = [7,3,11,1,5,null,13]
   输出：true
   解释：每个节点的左右子树高度差都不超过 1。

.. code-block:: text

   输入：root = [7,3,11,1,null,null,13,0]
   输出：false
   解释：节点 3 的左子树高度为 2，右子树高度为 0，高度差为 2，因此整棵树不平衡。
