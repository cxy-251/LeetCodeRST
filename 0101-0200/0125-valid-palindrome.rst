...
:教学重点: 跳过非字母数字字符、大小写归一化、不构造清洗串

题目重述
--------

只保留字符串中的 ASCII 英文字母和十进制数字，并忽略字母大小写。判断得到的字符序列是否为回文。若全部字符都被忽略，清洗后的空序列视为回文。
...
不同语言的 ``isalnum`` 或 Unicode 分类可能接受非 ASCII 字母数字，大小写折叠规则也不完全一致。题目字符域按 ASCII 定义时，显式判断 ``0-9``、``A-Z``、``a-z`` 可保证九语言行为一致。
