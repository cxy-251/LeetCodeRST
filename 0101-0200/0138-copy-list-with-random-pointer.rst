...
:重点: 节点身份映射、随机引用、副本独立、空链表

题目重述
--------

给定一条链表的头节点 ``head``。每个节点包含 ``val``、``next`` 和 ``random`` 三个字段，其中 ``random`` 可以指向链表中的任意节点，也可以为 ``null``。返回整条链表的深拷贝：每个原节点都必须对应一个全新副本，副本之间的 ``next`` 与 ``random`` 关系和原链表完全一致，并且返回结构中不能包含任何指向原节点的引用。空链表返回 ``null``。

链表节点数在 ``0..1000`` 范围内，节点值在 ``-10^4..10^4`` 范围内；每个非空 ``random`` 都指向原链表中的某个节点。
...
.. code-block:: text
   ...
   解释：a、b、c 都是新对象；即使 A 与 B 的值相同，它们仍必须对应两个不同副本。
