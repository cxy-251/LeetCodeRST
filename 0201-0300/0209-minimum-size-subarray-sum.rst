...
:题号: 0209
:难度: Medium
:主题: 数组、滑动窗口、双指针、最短连续区间
:原题: `LeetCode 0209 <https://leetcode.com/problems/minimum-size-subarray-sum/>`_
:访问状态: Available
:教学重点: 全正数单调性、持续收缩、最短性证明、宽整数求和

精确契约
--------

给定正整数 ``target`` 和一个只包含正整数的数组 ``nums``，返回元素和大于或等于 ``target`` 的
最短非空连续子数组长度。不存在这样的子数组时返回 ``0``。

本文采用以下合同：

* ``target > 0``；
* 每个 ``nums[i] > 0``，全正数是主算法正确性的必要前提；
* 子数组必须连续且非空；
* 只返回最短长度，不返回区间或元素；
* 多个区间具有相同最短长度时，返回该长度即可；
* 输入数组只读；
* 空数组作为扩展边界返回 ``0``。

固定宽度语言使用比单个元素更宽的累加类型。返回值最多是数组长度，仍使用普通整数。
...
最短区间是 ``[4,3]``。长度为 4 的 ``[2,3,1,2]`` 只是算法遇到的第一个可行窗口，不能立即停止。
...
加入最后一个元素后，窗口依次出现长度 4、3、2、1 的可行状态。若只移动一次左端，会错误返回 4。

为什么正数前提不可删除
----------------------

窗口右端加入正数时，窗口和严格增加；左端移除正数时，窗口和严格减少。因此：

* 和不足时，移动左端只会让和更小，唯一有希望的动作是扩展右端；
* 和已经达标时，继续扩展右端只会更长，当前右端下应持续移动左端寻找更短窗口；
* 一旦移除某个正数后和降到 ``target`` 以下，再继续移除只会更小，可以停止收缩。

若允许负数，加入新元素可能让和下降，移除左端可能让和上升，以上方向判断全部失效。例如
``target=3, nums=[2,-1,2,1]`` 中，窗口和不再随边界单调变化。本章不把同一算法推广到该模型。
