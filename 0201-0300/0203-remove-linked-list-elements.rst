...
给定一个单链表头指针 ``head`` 和整数 ``val``，
删除链表中所有节点值等于 ``val`` 的节点，并返回删除后的新头节点。
...
* 除删除节点的连接外，不改变保留节点的值；
* C 实现假设链表节点由当前调用拥有，删除节点时执行 ``free``；
  其他平台适配器按各自运行时或评测所有权规则只重接引用。

``ListNode`` 由平台提供，本文不重复声明节点类型。Julia 复用仓库的可变 ``ListNode``，R 复用以
``environment`` 表达共享引用语义的 ``new_list_node`` 构造器；
两者都必须使用函数返回值作为删除后的新头。
