...
:题号: 0208
:难度: Medium
:主题: Trie、前缀树、可变对象、资源生命周期
:原题: `LeetCode 0208 <https://leetcode.com/problems/implement-trie-prefix-tree/>`_
:访问状态: Available
:教学重点: 路径语义、终止标记、共享前缀、对象所有权与释放

精确契约
--------

设计一个 Trie（前缀树）对象，支持按调用顺序执行三种操作：

``insert(word)``
   把一个由小写英文字母组成的单词加入当前 Trie。重复插入同一个单词不会改变查询结果。

``search(word)``
   仅当此前插入过完整单词 ``word`` 时返回 ``true``。只存在相同字符路径还不够，路径终点还必须带有单词终止标记。

``startsWith(prefix)``
   只要 Trie 中存在某个已插入单词以 ``prefix`` 开头就返回 ``true``。该操作只要求路径存在，不要求路径终点是完整单词。

对象具有状态：后一次查询必须看到此前所有成功插入留下的节点与终止标记。官方字符域是 ``a`` 到 ``z``；
十语言实现都把字符映射到 ``0..25``。正文不把非法字符当成题目输入，也不声称当前数组下标实现支持任意 Unicode。

官方操作参数非空。本文额外约定空字符串语义，便于说明根节点：

* ``insert("")`` 把根节点标记为一个完整单词；
* ``search("")`` 返回根节点的终止标记；
* ``startsWith("")`` 总为 ``true``，因为空前缀对应根路径。

节点模型
--------

每个节点表示“从根沿某段字符路径到达的位置”，保存两类状态：

``children[26]``
   第 ``i`` 个子链接对应字母 ``'a' + i``。链接为空表示该字符路径尚不存在。

``is_word``
   表示从根到当前节点的完整字符序列是否曾被成功插入。它与“当前节点存在”是两个不同事实。

例如依次插入 ``apple`` 与 ``app`` 后：
...
星号表示 ``is_word=true``。路径 ``app`` 在第一次插入 ``apple`` 后已经存在，但直到插入 ``app``，
第三个字符节点才成为单词终点。
...
``search("app")`` 第一次返回假，说明路径存在不等于完整单词存在。

共享前缀
~~~~~~~~

插入 ``car``、``card``、``care`` 后，三个单词共享 ``c -> a -> r`` 路径，只在后续分支或终止标记上不同：
...
连续两次 ``insert("tea")`` 不应创建第二条独立路径，也不应让节点数量按重复次数增长。第二次扫描只复用既有节点，
并把已经为真的终止标记再次设为真。
...
#. Trie 中存在路径 ``p``，当且仅当 ``p`` 是某个成功插入单词的前缀，或 ``p`` 本身是已插入空串后的根路径；
#. ``v.is_word=true``，当且仅当完整字符串 ``p`` 曾成功插入；
#. 两个单词的公共前缀只对应一组共享节点，不因重复插入复制；
#. 查询操作不修改节点、子链接或终止标记。

根节点对应空前缀。它始终存在，所以 ``startsWith("")`` 为真；根的 ``is_word`` 只由是否插入空串决定。
