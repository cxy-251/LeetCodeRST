...
需要返回任意一个包含全部课程的合法学习顺序。若不存在这样的顺序，返回空数组。

本文采用以下合同：

* 一条先修对建立有向边 ``prerequisite -> course``；
* 每条输入边都保留，重复边同时增加邻接项和目标课程入度；
* 返回顺序必须恰好包含每门课程一次；
* 对每条边 ``u -> v``，``u`` 在返回数组中必须早于 ``v``；
* 合法拓扑序通常不唯一，不能把某一个示例数组当成唯一正确答案；
* 图中有自环或其他有向环时，不能返回已经处理出的部分前缀，必须返回空结果；
* 输入先修表保持只读；
* ``numCourses=0`` 作为扩展边界返回空数组，此时它同时表示合法空顺序。

建边与入度
----------

先修对 ``[1,0]`` 表示先完成 0，再学习 1，因此：
...
``indegree[v]`` 初始表示完整图中进入课程 ``v`` 的边数。Kahn 算法处理一门课程后，删除它的全部出边；
此时入度继续表示来自尚未处理课程的剩余先修边数。
...
合法输出可以是 ``[0,1,2,3]``，也可以是 ``[0,2,1,3]``。课程 1 与 2 之间没有先后约束。
...
队列中无法出现环内的第一个课程。即使图中另有孤立课程被处理，也不能返回部分数组。
...
课程 1 的入度是 2，课程 0 的邻接表中也有两个指向 1 的项。处理课程 0 时执行两次递减，
只有第二次使入度从 1 变为 0，课程 1 仍只入队一次。
