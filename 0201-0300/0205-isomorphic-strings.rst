...
给定两个字符串 ``s`` 和 ``t``，判断能否把 ``s`` 中的每个字符替换为一个字符，
使替换结果恰好等于 ``t``。
...
这等价于：在 ``s`` 实际出现的字符集合与 ``t`` 实际出现的字符集合之间，存在一个
保持每个位置对应关系的双射。

官方字符域是 ASCII。C、C++、Rust 与 Go 因此按字节处理；Java、TypeScript 与 C#
按 UTF-16 代码单元处理；Julia 与 R 适配器按 Unicode 码点物化。对官方 ASCII 输入，
这些单位完全一致。超出 ASCII 时，各语言适配器的“字符”边界可能不同，本文不声称
它们对任意 Unicode 字素簇具有相同语义。
...
``foo`` 与 ``bar`` 不同构。第一个 ``o`` 要映射到 ``a``，第二个 ``o`` 又要映射到
``r``，同一源字符出现了两个目标。
...
虽然每个源字符只提出一个目标，但 ``b`` 已被源字符 ``b`` 占用，``a`` 已被源字符
``a`` 占用。只检查 ``s -> t`` 的单向一致性会错误返回真。
