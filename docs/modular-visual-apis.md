...
- `lights-beams`
  效果：整条视频使用发光束线型 WebGL 中间层，风格更接近音乐可视化 / 舞台灯束 / 沉浸式空间氛围案例。
- `rubiks-solver`
...
- `lights-launch`
  效果：用于第二页按钮启动 `Lights` 风格的发光束线主体层。
- `lights-beams`
  效果：发光束线主体层，当前使用 `Three.js + WebGL`。
- `rubiks-launch`
...
- `lights-beams`
  可调：variant、beamCount、beamLength、beamThickness、orbitRadius、motionSpeed、spread
- `rubiks-solver`
