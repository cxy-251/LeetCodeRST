...
这项检查只证明字节一致，不证明内容逻辑正确。代码运行、文档检查和状态一致性仍需在上传前单独完成。
...
#. 差异文件集合与计划完全一致，没有临时测试文件或无关修改；
#. 从新的 ``main`` 重新读取关键文件，而不是只相信写接口返回成功；
#. 状态文件、README、索引和正文指向同一完成范围；
#. CI 状态与本地验证分别报告，没有把“没有检查”写成“检查通过”。
...
* 本地 ``git hash-object`` 与远端 blob ID 一致，可以证明验证过的字节就是上传的字节；
