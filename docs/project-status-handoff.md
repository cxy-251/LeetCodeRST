...
21. 屏显稿当前已进一步扩成“技术卡片”风格：
   - 允许 4 条 bullets，而不是强制只有 3 条
   - 会优先补“这篇论文的新意是什么”“不是新模型而是新分类/新证明/新边界”这类判断
   - 综述类和理论类论文会自动补术语解释，例如 `pointed Kripke model`、`epistemic action`
22. 朗读稿与屏显稿当前已明确分工：
...
25. `data/papers/paper-urls.csv` 现已支持 `paper_url,status` 双列模式：
   ...
   - 单篇成功后会立即把当前行写回成 `processed`
26. `LM Studio` 总结链路已从“字段缺一个就整批失败”改成：
   - 先用规则保底稿补齐缺失字段
   - 再做最终校验
   - 这样 `missing required field: value` 不会再轻易把整批中断
27. 总结文案已开始强制去掉“先收藏 / 值得先读 / 推荐去看论文”这类引流口吻，改成“看完短视频就能抓住主线”的知识讲解风格
...
58. `lights-beams` 的前进参照物已从中轴转向两侧：
   - 中轴导向线已移除
59. 论文脚本落地逻辑已优化：
   - `hook` 页正文不再显示 `arXiv / 分类` 元信息
   - `method` 页改成“核心方法”短句
   - `value` 页正文与 bullets 更聚焦论文价值点，不再混入发布时间、方向标签这类弱信息
60. `produce:paper-urls:donut` 的默认 seed 已改为按批次自动生成：
   - 不传 `--seed` 时，不同批次会得到不同甜甜圈配色和旋转参数
   - 显式传入相同 `--seed` 时，结果仍然可复现
59. `snake-grid` 已从单食物升级为确定性多食物池：
   ...
   - 地图已对 `snake-grid` 单独收缩，不再沿用生命游戏那套大棋盘
   - 食物已拆成低/中/高价值三档，并映射为不同颜色
   - 默认食物色带已改成暖色/洋红系，避免与蛇身薄荷色混淆
   - 蛇当前不会再穿过墙壁，边界会被当作实体墙处理
   - 寻路核心已改成“优先能回尾、否则保留最大安全区”的安全策略，避免把蛇头带进死胡同
   - `snake-grid` 当前已改成双策略：默认使用“survival-chase”生存优先路线，保留 “safe-loop” 作为保底/展示解法选项
   - 默认移动速度已提升，但地图尺寸与当前布局保持不变
   - 已移除内部隐藏的半速步进，当前速度会直接跟 `Move Interval` 对齐
59. 当前已新增源码交付打包入口：
   - `npm run release:package`
   - 基于 git 已跟踪文件生成可交付目录
   - 自动排除 `.git/`、`node_modules/`、`output/`、`.env`
   - 若系统存在 `zip` 命令，会额外输出 zip 压缩包
60. `rubiks-solver` 已做一轮高级化升级：
   - cubie 从单一彩色方块改成“深色主体 + 独立贴纸层”
   - 引入圆角几何、内核、接触阴影与 halo
   - 默认镜头改成更展陈化的透视环绕
   - 默认参数已收紧到更接近成品演示的状态
...
72. `LM Studio` 适配层已增强容错：
   - 优先请求结构化 JSON 输出
   - 若模型返回脏 JSON，会先做本地容错解析
   - 若仍失败，会调用本地模型做一次“JSON 修复”再落回统一脚本结构
