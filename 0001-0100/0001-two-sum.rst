...
:接口: ``twoSum(nums, target) -> [i, j]``，返回零基下标
:教学重点: 二元组搜索空间、补数等价转换、查询先于写入、原始下标保留

题目重述
--------

给定整数数组 ``nums`` 和整数 ``target``，找出两个不同位置 ``i``、``j``，使
``nums[i] + nums[j] == target``，并返回这两个位置的零基下标。题目保证恰好存在一个
有效答案，因此实现可以在首次命中时直接返回；返回 ``[i, j]`` 或 ``[j, i]`` 都合法。

同一个数组位置不能使用两次。即使 ``target`` 等于某个元素的两倍，也必须在数组中找到
两个不同下标，例如 ``[4, 4]`` 才能组成 8，单独一个 4 不能与自己配对。算法不需要修改
``nums``，数组长度为 ``2..10^4``，元素和目标值位于 ``-10^9..10^9``。
...
.. code-block:: text
   ...
   输出：[0, 2]
   解释：两个 4 的值相同，但下标不同；下标 0 不能重复使用两次。

负数与零同时出现：

.. code-block:: text

   输入：nums = [0, -7, 3, 7], target = 0
   输出：[1, 3]
   解释：-7 + 7 = 0；不能因为数组中存在 0 就返回同一个下标两次。
...
出现位置或其他历史位置都能组成合法下标对。当前 C++ 和九语言实现直接覆盖旧值，因此保存
最近一次历史下标；查询与写入的先后顺序才是避免当前位置匹配自己的关键条件。
...
九语言统一采用一次遍历哈希表。它在顺序扫描过程中直接维护题目需要的反向索引，不需要
排序，也能立即返回原始下标。
