...
:重点: 十进制表示、负号语义、末位零、镜像数字

题目重述
--------

给定 32 位有符号整数 ``x``，判断它的十进制表示从左向右和从右向左读取是否完全相同。

``x`` 位于 ``[-2^31, 2^31 - 1]``。负数的十进制表示包含开头的负号，因此负数都不是回文数。数字 ``0`` 是回文数；非零且末位为 ``0`` 的整数不是回文数，因为其十进制表示没有前导零。
...
.. code-block:: text
   ...
   输入：x = 0
   输出：true
   解释：单个数字 ``0`` 的正向和反向表示相同；它是末位零规则中的合法例外。
...
半反转保持常数工作空间，只处理约一半数字，并从算法结构上避免完整反转溢出，因此作为标准入口和 C++
主解法。
