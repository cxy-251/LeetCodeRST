...
:重点: 一基闭区间、区间内逆序、区间外顺序、结果头节点

题目重述
--------

给定单链表 ``head`` 和一基位置 ``left``、``right``，反转从第 ``left`` 个节点到第 ``right`` 个节点的闭区间，并返回结果链表。区间外节点的相对顺序保持不变。

链表节点数在 ``1..500`` 范围内，节点值在 ``-500..500`` 范围内，并保证 ``1 <= left <= right <=`` 链表长度。一次遍历完成是附加挑战。
...
第 2 到第 5 个节点 ``8,7,6,5`` 被整体反转，首节点 9 保持不动。
