...
:重点: 七种符号值、加法写法、六种减法组合、规范输入

题目重述
--------

给定一个表示正整数的合法罗马数字字符串 ``s``，返回其对应的整数。

基本符号及数值为 ``I=1``、``V=5``、``X=10``、``L=50``、``C=100``、``D=500``、``M=1000``。通常较大或相等的符号写在较小符号左侧并将数值相加；只有以下六种减法组合例外：``IV=4``、``IX=9``、``XL=40``、``XC=90``、``CD=400``、``CM=900``。

``s`` 的长度位于 ``[1, 15]``，只包含上述七种字符，并且保证是 ``[1, 3999]`` 范围内整数的合法罗马数字表示。
因此本题只需转换合法输入，不需要验证或修复 ``IC`` 这类不符合规范的字符串。
...
.. code-block:: text
   ...
   输入：s = "CDXLIV"
   输出：444
   解释：CD 表示 400，XL 表示 40，IV 表示 4，总和为 444。
...
.. code-block:: text
   ...
   输入：s = "MCMXLIV"
   输出：1944
   解释：M 表示 1000，CM 表示 900，XL 表示 40，IV 表示 4。
