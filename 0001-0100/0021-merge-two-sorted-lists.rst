...
:教学重点: 有序头节点选择、虚拟头节点、尾指针推进、节点复用、剩余链表整体接入

题目重述
--------

给定两条按非递减顺序排列的单链表，合并为一条同样非递减的链表并返回头节点。主解法复用原节点，只修改
``next``；当两个头节点值相等时优先选择 ``list1``，形成确定的稳定顺序。
...
若 ``list1`` 为空，结果直接是 ``list2``。若比较过程中一条链表先耗尽，另一条剩余有序后缀可以整体接入。
