...
:重点: 等值分支、输入位置使用、唯一数值序列、重复排列消除

题目重述
--------

给定可能包含重复值的整数数组 ``nums``，返回所有不同的排列。每个输入位置在一个排列中恰好使用一次；由不同下标产生但数值序列完全相同的排列只保留一个。答案顺序不限。

约束为 ``1 <= nums.length <= 8``、``-10 <= nums[i] <= 10``。
...
交换两个值相同的 ``2`` 不会产生新的数值排列，因此只有三种答案。
