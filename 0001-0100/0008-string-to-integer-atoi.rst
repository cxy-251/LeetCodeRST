...
:题号: 0008
:难度: Medium
:主题: 字符串、解析、有限状态机、溢出边界
:原题: `LeetCode 0008 <https://leetcode.com/problems/string-to-integer-atoi/>`_
:重点: 前导空格、可选符号、连续数字前缀、停止规则、32 位钳制

题目重述
--------

给定字符串 ``s``，按照以下顺序把其开头解析为 32 位有符号整数：

#. 跳过字符串开头连续出现的普通空格；
#. 读取至多一个 ``'+'`` 或 ``'-'`` 作为符号；
#. 从当前位置连续读取十进制数字，忽略数字部分开头的零；
#. 遇到第一个非数字字符或字符串结束时停止；若没有读到任何数字，返回 ``0``；
#. 若解析值小于 ``-2^31``，返回 ``-2^31``；若大于 ``2^31 - 1``，返回 ``2^31 - 1``。

``s`` 的长度位于 ``[0, 200]``，可能包含英文字母、数字、空格、``'+'``、``'-'`` 和 ``'.'``。
...
.. code-block:: text
   ...
   输入：s = "   -001204x7"
   输出：-1204
   解释：跳过三个空格和负号后读取连续数字 001204，在字符 x 处停止，后面的 7 不再参与解析。
