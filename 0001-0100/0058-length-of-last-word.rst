...
:重点: 尾部空格、最后非空片段、正反向状态、字符单位

题目重述
--------

给定只包含英文字母和普通空格的字符串，返回最后一个单词的长度。单词是连续字母组成的最大非空片段；字符串保证至少包含一个单词，末尾可能有多个空格。
...
输入字符域只有 ASCII 字母和空格。字节、UTF-16 代码单元及 Unicode 码点对这些字符一一对应，各语言按索引或字符数组计数都会得到同一长度。
