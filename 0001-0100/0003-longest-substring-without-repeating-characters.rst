...
:接口: ``lengthOfLongestSubstring(s) -> int``
:教学重点: 连续子串搜索空间、合法窗口、最后位置索引、左边界只能前进不能回退

题目重述
--------

给定字符串 ``s``，返回其中不包含重复字符的最长连续子串长度。这里要求的是**子串**而不是
子序列，因此答案必须对应原字符串中的一个连续区间；只返回长度，不需要返回子串本身。

空字符串答案为 0。字符可以是英文字母、数字、符号和空格，大小写不同的字符视为不同字符。
题目给出的字符范围可按单字节 ASCII 处理；若迁移到完整 Unicode 字符域，需要按语言的字符
或码点语义调整遍历和索引结构。
...
.. code-block:: text
   ...
   输入：s = "a b a"
   输出：3
   解释："a b"、" b " 和 "b a" 的长度都是 3；重复空格或重复字母都会收缩窗口。
