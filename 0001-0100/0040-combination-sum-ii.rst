...
:重点: 每个位置至多使用一次、重复值去重、正数剪枝、唯一组合

题目重述
--------

给定正整数数组 ``candidates`` 和正整数 ``target``。每个数组位置在一个组合中最多使用一次，返回所有元素和等于 ``target`` 的不同组合；组合内部顺序不重要，答案顺序不限。数组可能含重复值，但数值相同的组合只能返回一次。

约束为 ``1 <= candidates.length <= 100``、``1 <= candidates[i] <= 50``、``1 <= target <= 30``。
...
三个 ``1`` 来自三个不同位置，因此 ``[1,1,1]`` 合法；选择任意一个 ``1`` 与 ``2`` 得到的数值组合都相同，只返回一个 ``[1,2]``。
...
本题限制每个数组位置最多使用一次。选择 ``index`` 后，下一层只能从后续下标开始，因此传入 ``index+1``；如果仍从当前下标开始，就会把同一个位置重复计入组合。
