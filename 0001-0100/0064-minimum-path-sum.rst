...
:重点: 起点终点计入、右下移动、最优子结构、边界累加

题目重述
--------

给定 ``m × n`` 非负整数网格 ``grid``，从左上角出发，每步只能向右或向下移动，返回到达右下角的最小路径和。路径和包含起点、终点以及经过的所有单元格。

约束为 ``1 <= m, n <= 200``、``0 <= grid[i][j] <= 200``。
...
路径 ``2 -> 1 -> 2 -> 1 -> 1`` 的和为 7，且不存在更小的合法路径。
