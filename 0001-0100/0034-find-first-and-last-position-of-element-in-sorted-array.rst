...
``binaryThenExpand`` 先二分得到下标 ``found``，再从这里分别向左和向右移动，直到相邻元素不再等于目标。
重复段长度为 ``r`` 时，扩展仍需要 ``O(r)`` 次操作。

若数组全部由目标值组成，二分虽然很快命中中间位置，随后仍要走到数组两端，总时间退化为 ``O(n)``。瓶颈已经
从“寻找目标”转移到“逐个穿过重复段”，因此边界本身也必须通过二分直接定位。

单调边界条件
~~~~~~~~~~~~

边界二分不再询问“中点是否等于目标”，而是寻找某个单调条件第一次成立的位置。
...
在有序数组中，这个条件沿下标方向呈现为一段 ``false`` 后跟一段 ``true``。第一个成立的位置就是
``lowerBound(target)``，即第一个不小于目标的下标。
...
第一个成立的位置是 ``upperBound(target)``，即第一个严格大于目标的下标。记它为 ``afterLast``，重复段右端
就是 ``afterLast - 1``。

直接搜索“严格大于”比搜索 ``target + 1`` 更准确，也不依赖整数加一，因此不会在目标达到整数上界时产生溢出。
...
* ``[0, low)`` 中的值都严格小于 ``target``；
* ``[high, n)`` 中的位置都满足值不小于 ``target``；
* 第一个满足条件的位置仍位于 ``[low, high]``。

若 ``nums[mid] < target``，中点及其左侧都不满足条件，令 ``low = mid + 1``。否则中点仍可能是第一个满足位置，
令 ``high = mid``。当 ``low == high`` 时，唯一候选就是左边界。

``upperBound`` 使用相同的区间结构，只把不满足条件的范围改为 ``nums[mid] <= target``。所有相等元素会被排除到
左侧，搜索最终停在重复段之后。
...
``lowerBound`` 定位第二段起点，``upperBound`` 定位第三段起点。若 ``lowerBound`` 越过数组，或该位置的值不等于
目标，第二段为空，答案就是 ``[-1, -1]``。

目标存在时，两次边界之间的元素全部等于目标，所以答案直接为：
...
``linearScan`` 逐个读取元素并直接维护首尾位置，正确但没有利用边界条件的单调性。

``binaryThenExpand`` 用普通二分删除了寻找任意目标位置的线性工作，但向两侧扩展仍会逐个穿过重复段。它说明
“命中即停止”的二分语义太弱，必须让搜索继续逼近边界。

``boundaryBinarySearch`` 分别寻找第一个 ``>= target`` 与第一个 ``> target`` 的位置，删除命中后的线性扩展。
公开入口采用该方法，两次搜索都只保留可能包含边界的一半区间。
