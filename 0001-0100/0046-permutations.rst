...
:重点: 位置决策、每个元素恰好使用一次、结果快照、输出规模

题目重述
--------

给定一个元素互不相同的整数数组 ``nums``，返回它的全部排列。每个排列必须包含所有输入元素且每个元素恰好出现一次；答案顺序不限。

约束为 ``1 <= nums.length <= 6``、``-10 <= nums[i] <= 10``，并保证所有元素不同。
...
.. code-block:: text
   ...
   输入：nums = [-1,0,4]
   输出：[[-1,0,4],[-1,4,0],[0,-1,4],[0,4,-1],[4,-1,0],[4,0,-1]]

三个不同元素共有 ``3! = 6`` 种排列；输出次序可以不同。
