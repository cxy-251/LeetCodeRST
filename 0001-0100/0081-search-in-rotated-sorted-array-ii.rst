...
:重点: 重复端点消歧、有序半区、目标存在性、最坏线性退化

题目重述
--------

给定一个由非递减数组在某个未知位置旋转得到的整数数组 ``nums``，数组中允许出现重复值。判断 ``target`` 是否存在于数组中，并尽量减少整体操作步骤。

约束为 ``1 <= nums.length <= 5000``、``-10^4 <= nums[i], target <= 10^4``。
...
端点、中点可能都等于 1，必须先消除重复值造成的半区歧义，才能保留包含 0 的位置。

.. code-block:: text

   输入：nums = [4,4,5,6,0,1,2,4], target = 3
   输出：false
