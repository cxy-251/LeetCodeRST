...
单节点 ``[0]`` 是数字零的完整表示，结果只保留和的有效数位，不会因为输入长度为一而额外
生成高位零节点。
...
输入没有共享节点。这种对象语义最清晰，也是本文 C++ 主解法采用的语义。
...
主解法选择新建结果链表，因为它保持输入结构、节点身份和返回所有权清晰，能够直接对应题目的
输入与输出语义。复用方案适合接口允许修改输入且需要减少节点分配的场景。
