...
:接口: ``addTwoNumbers(l1, l2) -> ListNode``
:教学重点: 逆序数位表示、逐位竖式加法、进位状态、输入结束条件、结果节点所有权

题目重述
--------

两个非空单链表 ``l1`` 和 ``l2`` 分别表示两个非负整数。每个节点保存一位十进制数字，
链表头部是最低位，沿 ``next`` 方向依次走向更高位。返回一个同样按低位到高位排列的新链表，
表示两数之和。

每个输入链表包含 ``1..100`` 个节点，节点值位于 ``0..9``。除数字 0 本身外，输入数字没有
多余的最高位零。两个整数可能具有不同位数；较短链表结束后，其缺失高位按 0 处理。若最高位
相加后仍有进位，结果必须额外创建一个节点。主解法不修改输入链表，也不把整条链表转换为固定
宽度整数。
...
.. code-block:: text
   ...
   输入：l1 = [0], l2 = [0]
   输出：[0]
   解释：结果仍由一个值为 0 的节点表示，不返回空链表，也不追加多余高位零。
