...
:教学重点: 元素分类、有效前缀、读写指针、尾部交换、顺序要求

题目重述
--------

给定整数数组 ``nums`` 和目标值 ``val``，原地移除所有等于 ``val`` 的元素并返回剩余数量 ``k``。调用者只检查
前 ``k`` 个位置，尾部内容任意。题目允许改变保留元素顺序，主解法仍选择稳定压缩以保持原相对顺序。
...
.. code-block:: text
   ...
   nums = [3,1,3,2,4,3], val = 3
   返回 k = 3
   有效前缀 = [1,2,4]

目标值不存在时 ``k`` 等于原长度；全部元素等于目标时 ``k = 0``。
