...
:重点: 问号匹配单字符、星号匹配任意长度、整串匹配、回退边界

题目重述
--------

给定字符串 ``s`` 和模式 ``p``，判断模式能否匹配整个字符串。普通小写字母只能匹配自身；``?`` 恰好匹配一个任意字符；``*`` 可以匹配任意长度的字符序列，包括空序列。只有 ``s`` 与 ``p`` 都被完整消费时才算匹配成功。

约束为 ``0 <= s.length, p.length <= 2000``；``s`` 只含小写英文字母，``p`` 只含小写英文字母、``?`` 和 ``*``。
...
回退时保持的具体不变量是：``s[0:matched_after_star]`` 已被当前星号及其之前的模式匹配，下一次从 ``star+1`` 重新尝试。``matched_after_star`` 每增加一位，就恰好把当前星号的覆盖长度从 ``k`` 改为 ``k+1``；长度更小的分配已经在此前的失配中排除。更早的星号位于已经固定的模式前缀中，当前星号可以吸收同样的位移，所以不需要同时保存它们的分配。
...
用自建示例追踪一次回退：``c*?y`` 先让 ``*`` 取空串，``?`` 匹配 ``a``；随后 ``y`` 遇到 ``b`` 失配，算法把
``*`` 的覆盖长度增加一位，回到 ``?`` 重新匹配 ``b``。后续 ``y`` 又遇到 ``x`` 时再次扩张星号，最终 ``*``
覆盖 ``ab``，``?`` 匹配 ``x``，``y`` 匹配末尾 ``y``。这正是代码中 ``j = star + 1``、``i = ++matched_after_star``
两句的作用。
