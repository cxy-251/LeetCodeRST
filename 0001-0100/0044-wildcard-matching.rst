...
线性方法保存最近一次看到的星号下标 ``star``，以及该星号当前已经覆盖到的字符串末端 ``matched_after_star``。首次遇到星号时先让它匹配空串；若后续普通匹配失败，就回到星号之后，并让该星号比上次多覆盖一个字符。
...
递归最坏指数级；动态规划为 ``O(mn)`` 时间和空间；贪心通常线性扫描，最坏回退仍受字符串与模式推进约束，时间 ``O(m+n)``、额外空间 ``O(1)``。
