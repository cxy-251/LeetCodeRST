...
:重点: 成对递归状态、空节点结构检查、同方向孩子、短路终止

题目重述
--------

给定两棵二叉树 ``p`` 和 ``q``，判断它们是否完全相同。完全相同要求每个对应位置同时为空或同时存在，并且对应非空节点值相等。左右孩子方向必须一致；仅有相同节点值集合或相同的无空标记遍历序列不够。两棵树只读。
