...
:重点: 两条竖线构成容器、短板决定高度、下标差决定宽度

题目重述
--------

给定长度为 ``n`` 的整数数组 ``height``。下标 ``i`` 处有一条竖线，它的两个端点是 ``(i, 0)`` 和 ``(i, height[i])``。选择两个不同下标 ``i < j``，两条竖线与横轴共同构成一个容器，其可容纳的水量为：

``(j - i) * min(height[i], height[j])``。

返回所有选择中能够容纳的最大水量。竖线不能倾斜。``n`` 位于 ``[2, 10^5]``，每个高度位于 ``[0, 10^4]``。
...
.. code-block:: text
   ...
   输入：height = [8, 1, 1, 1, 10, 1, 1, 1, 8]
   输出：64
   解释：选择下标 0 和 8，宽度为 8，较短竖线高度为 8，水量为 8 * 8 = 64；最高的 10 在中间，反而不属于最优边界。
...
.. code-block:: text
   ...
   输入：height = [0, 6, 2, 5]
   输出：10
   解释：选择下标 1 和 3，宽度为 2，较短高度为 5，水量为 10。
