...
:重点: 恰好四段、数值范围、前导零、完整使用全部字符

题目重述
--------

给定只包含数字的字符串 ``s``，在不改变字符顺序且不删除字符的前提下插入三个点，返回所有可能的合法 IPv4 地址。地址必须恰好包含四段，每段长度为 1 到 3，数值在 ``0..255`` 范围内；除单独的 ``0`` 外，段不能有前导零。答案顺序不限。
...
这四种分段都完整使用了原字符串，且每段都在 ``0..255`` 范围内并没有非法前导零。
