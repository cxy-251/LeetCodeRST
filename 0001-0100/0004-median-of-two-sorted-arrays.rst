...
:教学重点: 合并序列中位位置、左右分割数量、较短数组二分、交叉边界、奇偶结果统一

题目重述
--------

给定两个分别按非递减顺序排列的整数数组 ``nums1`` 和 ``nums2``，返回它们合并后的中位数。
其中一个数组可以为空，但两个数组的总长度大于零。实现需要达到对数级时间复杂度，并保持两个
输入数组不变。

总长度为奇数时，中位数是合并序列正中间的一个元素；总长度为偶数时，中位数是中间两个元素
的平均值。
...
九语言统一采用二分分割。它满足题目要求的对数复杂度，并且只读取输入数组和分割线附近的常数个值。
