...
:重点: 全局严格递增、虚拟一维序列、坐标换算、对数搜索

题目重述
--------

给定 ``m × n`` 整数矩阵 ``matrix``。每行按非递减顺序排列，并且每行第一个元素严格大于上一行最后一个元素。判断 ``target`` 是否存在于矩阵中；算法时间复杂度必须为 ``O(log(m * n))``。
...
.. code-block:: text
   ...
   输入：
   matrix = [[2,4,8],[12,15,19],[25,31,40]]
   target = 15
   输出：true

矩阵按行连接后形成严格递增序列，15 位于第二行中间。
