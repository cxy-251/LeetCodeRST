...
最直接的多路扩展是先合并第 0、1 条链表，再把结果与第 2 条合并，依次处理全部链表。``mergeSequentially``
始终得到正确结果，但早期进入结果的节点会在以后每次归并中被重新扫描。
...
两路归并本身已经是线性操作，退化来自归并树失衡：一个不断增长的长链表反复与短链表合并。要减少重复扫描，
需要让规模相近的结果优先合并。
...
遗漏任何链表。每条原链表最终沿归并树进入根节点，所有输入节点都被保留一次；每次两路归并又保持非递减顺序，
所以根节点对应的链表就是完整有序结果。

公开入口采用这一方法。它直接复用第 21 题的两路归并，并重新连接原节点。
...
分治与最小堆都把单个节点的比较成本控制在 ``O(log k)`` 级别：

* 分治让节点沿平衡归并树逐层向上，每层最多被扫描一次；
* 最小堆让节点入堆、出堆各一次，每次堆操作只面对至多 ``k`` 个当前表头。

分治反复调用简单的两路归并，常数较小；最小堆始终显式维护所有链表的当前最小候选。二者都复用原链表节点，
区别在于组织候选的方式，而不是最终复杂度。
