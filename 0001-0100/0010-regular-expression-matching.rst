...
:教学重点: 完整匹配、后缀状态、点号单字符匹配、星号零次或多次、依赖顺序

题目重述
--------

给定字符串 ``s`` 和模式 ``p``，判断模式能否匹配字符串的全部字符。模式中的普通小写字母只匹配自身，
``.`` 匹配任意单个字符，``*`` 修饰它前面的元素并允许该元素出现零次或多次。

这里要求完整匹配：模式和字符串必须同时耗尽。题目保证模式合法，``*`` 不会出现在开头，并且总有可修饰的前一个元素。
