...
:重点: 删除整个重复值段、仅保留出现一次的值、头部重复、结果顺序

题目重述
--------

给定按非递减顺序排列的单链表 ``head``，删除所有出现次数大于 1 的值对应的全部节点，只保留原链表中出现恰好一次的值，并返回结果链表。
...
值 1、3、5 都出现多次，因此对应节点全部删除；只有 2 和 4 保留。
...
主 C++ 实现只修改 ``next``，保留节点的身份和相对顺序；被绕过的输入节点由调用方管理，本函数不负责释放它们。
