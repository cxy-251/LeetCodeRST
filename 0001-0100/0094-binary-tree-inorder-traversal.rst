...
给定二叉树根节点 ``root``，按“左子树、根节点、右子树”的中序顺序返回所有节点值。空树返回空数组。递归实现直接，使用迭代方式完成是附加挑战。

树的节点数在 ``0..100`` 范围内，节点值在 ``-100..100`` 范围内。
...
.. code-block:: text
   ...
   输入（层序）：[4,2,6,null,3,5]
   输出：[2,3,4,5,6]

节点 4 的左子树中序结果为 ``[2,3]``，随后访问根 4，再访问右子树 ``[5,6]``。
...
用自建树 ``[4,2,6,null,3,5]`` 追踪显式栈（右端是栈顶）：先沿左链压入 ``4``、``2``，弹出 ``2`` 后转向它的右子树 ``3``，于是输出前缀依次为 ``[2,3]``；随后弹出 ``4`` 并转向右子树。对 ``6`` 再沿左链压入 ``6``、``5``，最后得到 ``[2,3,4,5,6]``。每次弹出前都已经完成该节点的左子树，正好对应中序定义。
