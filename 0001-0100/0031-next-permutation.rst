...
:重点: 下一个字典序排列、最大排列回绕、原地修改、常量额外空间
...
若存在比当前排列更大的排列，应得到其中字典序最小的一个；若当前排列已经是所有排列中字典序最大的，则把数组变为字典序最小的非递减排列。必须原地修改数组，并且只使用常量级额外空间。

``nums`` 的长度位于 ``[1, 100]``，每个元素位于 ``[0, 100]``；数组中可以存在重复值。
