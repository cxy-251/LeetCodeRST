...
:重点: 三种取值、原地排序、禁止库排序、单遍常量空间进阶

题目重述
--------

给定只包含 ``0``、``1``、``2`` 的数组 ``nums``，分别代表红、白、蓝三种颜色。把数组按 ``0``、``1``、``2`` 的顺序原地排序，不能调用语言提供的通用排序函数。

约束为 ``1 <= nums.length <= 300``。使用一次扫描和 ``O(1)`` 额外空间完成是附加挑战。
