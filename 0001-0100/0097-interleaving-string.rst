...
:重点: 双前缀状态、来源二选一、长度守恒、左到右覆盖

题目重述
--------

给定 ``s1``、``s2``、``s3``，判断是否能在保持 ``s1`` 与 ``s2`` 各自字符相对顺序的前提下，交替取出两者全部字符组成 ``s3``。可以连续从同一来源取多个字符；必须恰好使用两个来源的全部字符。
...
在示例中，当目标前缀到达多个 ``b`` 时，两个来源都可能提供相同字符。DP 不贪心选择某一来源，而是保留所有可达的 ``(i,j)``；后续字符会自动淘汰无法继续匹配的路径，因此不会因局部同字符选择而错过答案。
