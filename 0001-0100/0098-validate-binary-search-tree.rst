...
:题号: 0098
:题名: Validate Binary Search Tree
:难度: Medium
:类型: Algorithms
:主题: 二叉搜索树、中序遍历、递归边界、显式栈
:原题: `LeetCode 0098 <https://leetcode.com/problems/validate-binary-search-tree/>`_
:教学重点: 全局严格约束、中序递增、前驱状态、整数边界

题目重述
--------

给定一棵二叉树的根节点 ``root``，判断它是否为合法的二叉搜索树。对于任意节点，其左子树中的所有值都必须严格小于该节点值，右子树中的所有值都必须严格大于该节点值，并且左右子树本身也必须满足相同条件。
...
节点 3 小于根 5，却位于根的右子树。只检查直接父子关系会漏掉该错误。
