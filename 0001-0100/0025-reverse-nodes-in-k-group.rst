...
:题号: 0025
:难度: Hard
:主题: 单链表、分组反转、半开区间、指针重连
:原题: `LeetCode 0025 <https://leetcode.com/problems/reverse-nodes-in-k-group/>`_
:教学重点: 完整组探测、先确认后修改、组内反转、组间重连、不完整后缀保留

题目重述
--------

给定单链表和正整数 ``k``，从头开始每 ``k`` 个连续节点组成一组并反转组内顺序。最后不足 ``k`` 个节点的后缀
保持原序。只能修改链接，不能交换节点值。
...
第一组完整，因此反转；剩余两个节点不足一组，链接不变。``k = 1`` 时每组只有一个节点，结果等于输入。
