...
:重点: 每个值保留一次、有序重复相邻、链表顺序、空链表

题目重述
--------

给定按非递减顺序排列的单链表 ``head``，删除重复节点，使每个不同值只出现一次，并返回去重后的链表。保留下来的不同值仍按原有非递减顺序排列。
...
本题永远保留头部值段的第一个节点，结果头不会因为去重而改变。只有当算法可能删除整个头部值段时才需要用哨兵统一处理前驱；本题只修改保留节点的后继链接即可。
