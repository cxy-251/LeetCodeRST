...
:重点: 符号位置、小数格式、指数整数、完整字符串验证

题目重述
--------

判断字符串 ``s`` 是否完整表示一个合法十进制数。底数可以是整数或小数，并可带一个前导 ``+`` 或 ``-``；指数部分可选，以 ``e`` 或 ``E`` 开始，指数可以带符号，但必须包含至少一个数字且不能含小数点。底数也必须至少包含一个数字。

约束为 ``1 <= s.length <= 20``，字符只可能是英文字母、数字、加号、减号或小数点。
