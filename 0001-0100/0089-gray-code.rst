...
:重点: ``2^n`` 个不同整数、相邻一位差、首尾一位差、起点为零

题目重述
--------

给定整数 ``n``，返回一个长度为 ``2^n`` 的整数序列。序列必须从 0 开始，并包含 ``0`` 到 ``2^n - 1`` 的每个整数恰好一次；任意相邻元素以及最后一个元素与第一个元素的二进制表示都必须恰好相差一位。任意满足条件的序列均可返回。
...
相邻元素异或后都是 2 的幂，最后的 8 与开头的 0 也只相差最高位。
