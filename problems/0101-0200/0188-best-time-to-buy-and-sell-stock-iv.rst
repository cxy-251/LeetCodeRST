...
``NEG`` 是安全的负无穷哨兵。固定宽语言取远离类型下界的值，并且只有在来源状态可达时才执行加减，
避免对最小整数直接减价格造成溢出。
...
* 输入只读，没有价格数组副本或路径输出；
* 固定宽语言使用 64 位利润状态，最终合法答案在官方范围内可转换为平台 ``int``；
* C 为 ``cash`` 和 ``hold`` 分配动态数组，任一分配失败都会释放已分配内存并返回 ``-1``；
  ``-1`` 位于合法非负答案域之外，是平台签名没有错误通道时的资源失败标记；
* Python 使用大整数安全哨兵；TypeScript 与 R 当前范围内全部整数都处于双精度精确整数区间；
* Rust 按平台签名消费 ``Vec<i32>`` 的所有权，但算法不修改元素；
* Julia 使用 ``Int64`` 状态和一基数组；R 使用一基索引并显式构造递减序列。
...
本题题解代码未运行、未编译、未对拍。完成了以下人工推演与静态语义检查：
...
* 固定宽语言对 ``NEG`` 做可达性检查后才加减价格，没有对类型最小值直接运算；
* C 的两次分配、部分失败清理、正常释放和 ``-1`` 资源错误标记逐项核对；
* Julia 使用 ``limit:-1:1``；R 仅在 ``limit>=1`` 的有限分支构造同方向递减 ``seq.int``；
* TypeScript/R 的最大利润与中间整数远低于 ``2^53``，保持精确；
* 所有实现只读价格数组，不排序、不交换、不保存交易路径。

剩余风险：十语言代码没有在目标平台实际编译或执行；平台签名、标准库版本和资源失败行为仅完成静态核对。
...
* 使用“恰好 t 次”状态时，答案是所有 ``cash[0..limit]`` 的最大值，而不是强迫完成 ``limit`` 次；
* 不可达状态不能初始化为 0，否则会允许无成本持股或凭空完成卖出；
* 使用最小整数作为负无穷后直接减价格可能溢出；应使用安全哨兵并检查可达性；
* 原地更新若改成递增 ``t``，``cash[t]`` 可能读到当天刚更新的 ``hold[t-1]``，破坏前一日语义；
* 无限交易贪心只能在 ``k>=floor(n/2)`` 时使用；有限小 ``k`` 不能收集所有上涨段；
...
* **依赖方向决定原地顺序**：递减下标确保读取前一日的 ``hold[t-1]``；
* **约束失效退化**：当交易额度超过物理可完成上限时，动态规划退化为正相邻差贪心；
