...
最优方案是选择金额 ``2``、``9`` 和 ``1``，总金额 ``12``。只观察相邻两间并选择较大值，可能先选择 ``7``，
随后丢失与 ``9`` 组合的机会，因此局部贪心不能保证全局最优。
...
若先执行 ``previous_two = previous_one``，再计算 ``previous_two + value``，选择当前房屋的分支就会错误读取
``F(i)`` 而不是 ``F(i-1)``。这等价于允许当前房屋与前一间房屋同时进入方案，破坏相邻排斥条件。
...
任意只使用前 ``i`` 间房屋的合法方案，对第 ``i`` 间房屋只有“选择”或“不选择”两种可能，不存在第三类。
...
根据引理一，全部合法方案只分为“最后一间不选”和“最后一间选择”两类；根据引理二、三，两类最优值分别是
``F(i-1)`` 与 ``F(i-2)+nums[i-1]``。在两类中取较大值，正好得到全部合法方案的最大金额：
...
随后旧 ``previous_one`` 被移入 ``previous_two``，而 ``current`` 被写入 ``previous_one``，因此推进后分别为
``F(i)`` 和 ``F(i+1)``，下一轮不变量成立。
...
``NULL`` 只在大小非正时作为扩展边界接受；官方非空输入不会触发该分支。状态使用 ``long long``，最终按题目范围
转换为 ``int``。
...
剩余风险主要是目标评测环境的具体语言版本、类型别名或函数签名可能与本文平台模型存在细微差异；核心状态转移、
边界处理和标准语法已按静态方式逐项核对。
...
答案：``previous_one`` 表示包含前一间房屋在内的前缀最优值，其中可能已经选择前一间。直接相加无法保证相邻约束。
选择当前房屋只能接到不含前一间的 ``previous_two``。
...
答案：这种方案属于“不选择最后一间”一类，并已经包含在 ``F(i-1)`` 中。``F(i-1)`` 允许它的最后一间也不选。
...
答案：空方案金额 ``0`` 始终是有效下界，状态无需负无穷初始化；同时最终答案不会为负。递推本身即使允许负数也可通过
空方案保持非负，但题目合同明确给出非负金额。
...
答案：计算 ``F(i)`` 只依赖 ``F(i-1)`` 与 ``F(i-2)``。更早状态已经被这两个前缀最优值完整概括，未来不会直接读取。
