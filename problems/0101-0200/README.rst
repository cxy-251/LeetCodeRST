...
公开题需要按冻结教学质量基线重写。``0127``、``0129–0147`` 已完成，
...
下一批单独返工 ``0148``：Medium，合计 2 分。该题需要用自底向上归并满足 ``O(n log n)`` 时间与常量核心
工作空间，证明定长 run 切分、稳定合并、逐轮覆盖、节点守恒和宽度终止，并为 Rust ``Box`` 与 R environment
给出非递归所有权路径。全部 12 道剩余返工完成前，``0166`` 保持暂停。
