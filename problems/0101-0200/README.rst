...
公开题需要按冻结教学质量基线重写。``0127``、``0129–0150`` 已完成，
...
下一批连续返工 ``0151–0152``：两道 Medium，合计 4 分。``0151`` 证明逆向扫描的单词覆盖与单空格输出，
核对 ASCII 边界、切片、拼接和返回物化；``0152`` 证明以当前位置结尾的最大/最小乘积状态、非空初始化、
负数翻转与 32 位乘积安全。全部 9 道剩余返工完成前，``0166`` 保持暂停。
