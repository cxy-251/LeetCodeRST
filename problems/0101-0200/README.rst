...
本目录按题号顺序保存 0101 至 0200 的题目教程。当前已完成 0101 至 0135。
...
常规验证只运行官方示例和少量关键边界：Python 运行、C++ 编译，其余语言静态检查。
随机对拍和穷举只用于明确的高风险疑点，不再作为普通题完成条件。
所有生产、审查和修复只允许直接在 ``main`` 形成一个原子提交。

下一动作
--------

下一批为 ``0136–0138``：Easy + Medium + Medium = 5。``0139`` 为 Medium，不再加入。
