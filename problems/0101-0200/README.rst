...
公开题需要按冻结教学质量基线重写。``0127``、``0129–0149`` 已完成，
...
下一批单独返工 ``0150``：Medium，合计 2 分。该题需要以合法 RPN 前缀的栈深和已完成子表达式为不变量，证明
操作数次序、最终唯一值与线性复杂度，并逐语言统一向零截断除法、整数解析、32 位中间值和 C 分配失败语义。全部
10 道剩余返工完成前，``0166`` 保持暂停。
