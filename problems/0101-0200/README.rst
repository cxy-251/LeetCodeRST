...
本目录按题号顺序保存 0101 至 0200 的题目教程。当前已完成 0101 至 0155、0160、0162 和 0164 至 0165；
0156 至 0159、0161、0163 已登记为 Premium 跳过。每道公开题只有一个自包含 RST，正文与十语言代码全部
直接内联，不使用 ``.inc`` 或 ``.. include::``。
...
``0129–0165`` 范围内共有 31 道公开可访问题曾被错误拆成题目 RST 与代码 ``.inc``。本次已把 310 个
语言代码块按固定顺序内联回对应唯一 RST，并删除 169 个已消费的 ``.inc`` 文件。题解算法、完成题数、
Premium 登记和下一题均未改变。

执行规则
--------

当前及后续题目统一执行 ``../../docs/FORWARD_RULES_0151_ONWARD.rst``：

* 每道公开题只创建一个 ``<题号>-<slug>.rst``；
* 原创重述、算法、证明、复杂度、验证和十语言代码全部直接内联；
* 禁止题目 ``.inc``、代码分片和 ``.. include::``；
* Premium 或题面不可可靠确认的题目只登记状态，不生成猜测性正文；
* 所有生成、抽查、规则维护和修复只允许直接在 ``main`` 形成一个原子提交。

下一动作
--------

下一批为 ``0166–0169``：``0166`` Medium、``0167`` Medium、``0168`` Easy、``0169`` Easy，合计 6 分。
