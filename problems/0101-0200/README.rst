...
本目录按题号顺序保存 0101 至 0200 的题目教程。当前已完成 0101 至 0140。
...
#. `0128. Longest Consecutive Sequence
   <0128-longest-consecutive-sequence.rst>`_
#. `0129. Sum Root to Leaf Numbers
   <0129-sum-root-to-leaf-numbers.rst>`_
#. `0130. Surrounded Regions <0130-surrounded-regions.rst>`_
#. `0131. Palindrome Partitioning
   <0131-palindrome-partitioning.rst>`_
#. `0132. Palindrome Partitioning II
   <0132-palindrome-partitioning-ii.rst>`_
...
#. `0138. Copy List with Random Pointer
   <0138-copy-list-with-random-pointer.rst>`_
...
* ``0140`` Hard = 4；
* 总分为 6；
* ``0141`` 为 Easy，加入后会超过本批预算。

执行规则
--------

本范围直接执行 ``../../docs/FORWARD_RULES_0101_0150.rst``、质量门、题目模板、解法与类型策略以及
RST 写作规范。每题保持精确契约、算法状态、正确性、复杂度、十语言实现和关键边界。

常规验证只运行官方示例和少量关键边界：Python 运行、C 与 C++ 严格编译，其他语言按风险选择运行或静态
检查。随机对拍和穷举只用于明确的高风险疑点，不再作为普通题完成条件。
固定每 50 题的全量审核已取消，后续只做批次自检与风险触发抽查。
所有生产、抽查和修复只允许直接在 ``main`` 形成一个原子提交。

下一动作
--------

下一批为 ``0141–0144``：Easy + Medium + Medium + Easy = 6。``0145`` 为 Easy，不再加入。
