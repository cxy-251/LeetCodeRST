...
* 用“唯一答案仍在当前闭区间”表达本题的双指针不变量；
* 区分不同下标与不同数值，允许重复值在两个位置组成答案；
* 题目一基返回与 Julia/R 自然一基、其余语言零基内部状态之间的接口映射。

强化
~~~~

* 复用 0015、0016 已出现的有序端点单调排除，本题不再需要外层锚点；
* 复用 0001 的 C 固定两整数结果事务，并把核心常数状态与固定大小返回载荷分开计算；
* 继续用 ``left<right`` 同时关闭终止性、不同位置和无符号右端递减风险。

关联题目
~~~~~~~~

* `0001. Two Sum <../0001-0100/0001-two-sum.rst>`_：0001 面对无序数组使用哈希，
  本题借助有序前提把额外工作空间降为常数。
