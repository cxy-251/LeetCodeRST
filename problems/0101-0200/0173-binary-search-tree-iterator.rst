...
``next`` 单次最坏可以沿一条左链工作 ``O(h)``，整个遍历中每个节点只入栈、出栈一次，因此均摊 ``O(1)``。

仓库适配器允许空根：构造后 ``hasNext`` 为假。平台提供 ``TreeNode``；Julia 与 R 复用仓库的引用节点模型。
...
``pushLeft`` 依次压入 ``node``、其左孩子、左孙子，直到左指针为空。最后压入的节点没有尚未进入的左孩子；
...
* C 的 ``realloc`` 使用临时指针，扩容前先检查容量乘法，构造失败释放对象与栈；
...
* **惰性迭代器**：只计算当前消费所需的结构前缀；
* **递归续点栈**：把“左子树返回后访问根”的调用帧保存为显式对象状态；
* **均摊分析**：单次可能沿左链工作，但每个节点只承担一次入栈成本；
* **跨语言可变对象**：Julia ``mutable struct``、R ``environment``、C 显式生命周期；
* **资源失败与功能合同分离**：固定平台签名可能没有资源错误通道。
