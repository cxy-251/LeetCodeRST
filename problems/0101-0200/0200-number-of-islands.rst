...
除 R 和 Rust 平台适配外，其余实现会让调用者观察到网格中的陆地被清零。Rust 平台签名
按值取得并消费网格，算法修改的是其拥有的网格值；R 普通字符矩阵的下标赋值把 ``grid``
重新绑定在当前调用帧，调用者原绑定不会改变，并可能产生 ``O(mn)`` 写时复制。这些适配器
差异必须与核心原地算法分开说明。
...
* R 对字符矩阵的局部赋值不会改写调用者绑定，并可能触发 ``O(mn)`` 写时复制；工作栈仍另外需要
  最坏 ``O(mn)``；
* 官方 ``m,n <= 300`` 给出 ``mn <= 90000``，支撑 Java/C#/Go 的编码坐标、岛屿计数和
  TypeScript 的精确整数运算；C 仍对扩展输入独立检查 ``size_t`` 容量；
* TypeScript 使用 ``string[][]`` 单字符元素，Julia 使用 ``Matrix{Char}``，R 使用字符矩阵；
  所有比较都针对 ASCII 字符 ``'0'`` 与 ``'1'``。
