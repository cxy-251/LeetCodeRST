...
当前两个邻居中数值更大的方向，可能在后续遇到巨大伤害；较小的当前收益可能通向安全路径。
因此状态必须概括“从某格出发完成剩余旅程的最低进入生命”，而不是只比较下一格数值。
...
固定宽语言使用 64 位状态，并把 ``INF`` 设为远小于类型上界的值。按扫描不变量，
每个真实格至少有一个有限合法后继，因此 ``min`` 结果总是有限值，``INF`` 不会参与减法。
最终答案在题目范围内可安全转换为平台整数返回类型。
...
由引理四和引理五，扫描结束时 ``dp[0]=need[0][0]``。状态定义就是进入起点前完成整条路径所需的最小生命，
因此返回值正确。
...
* C 分配 ``n+1`` 个 64 位状态并在返回前释放；分配失败返回 0 作为平台外资源错误标记，
  合法答案始终至少为 1；
* C++、Java、Rust、Go、C# 与 Julia 使用 64 位中间状态；
* Python 整数任意精度；TypeScript/R 在当前约束下保持精确整数；
* R 若输入已经是矩阵，滚动状态为 ``O(n)``；适配器不额外复制整张矩阵；
* Julia 和 R 都使用显式负步长逆序，避免把递减范围误写为空或方向错误。
...
* **边界哨兵设计**：只开放一个虚拟终点，避免非法路径逃出状态空间；
* **一维逆向滚动不变量**：同一数组槽位在更新前表示下方、更新后表示当前；
* **宽类型中间运算**：先提升再减法，最终再转换平台返回类型。
