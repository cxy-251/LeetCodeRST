...
返回对应的 Excel 列号。输入只包含 ASCII 大写字母，因此本题按字节或 UTF-16 代码单元读取都不会拆分字符。
...
* 结果不超过 ``2^31-1``。TypeScript 和 R 的双精度数值远在精确整数范围内；固定宽语言使用 32 位整数也合法。
...
* **Horner 折叠**：用单个累计值替代显式幂；
* **无零位进制解码**：数位域是 ``1..26``，但高位折叠仍是普通乘加；
* **编码/解码非对称步骤**：编码要减一，解码直接读取数位；
* **适配器物化成本**：相同算法在 R 与 Julia 的字符串访问成本不同。
