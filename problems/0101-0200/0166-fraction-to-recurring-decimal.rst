...
不能因为商数字重复就立即判定循环。例如 ``1 / 17 = 0.(0588235294117647)``，其中相邻两个数字 8
分别由余数 15 和 14 产生；第二个 8 出现时，余数状态尚未重复。真正决定未来数字序列的是当前余数。
只有同一个余数再次出现，才说明长除法状态完全重复。
...
* Java、C# 的 ``StringBuilder.Insert``，Rust 的 ``String.insert``，Go/Julia 的字节搬移
  均有一次 ``O(L)`` 成本。
...
* ``2/3``：在 ``"0."`` 后记录余数 2；``20=6*3+2`` 生成数字 6 后又回到余数 2，
  因此在数字 6 前插入左括号，得到 ``"0.(6)"``。
* ``1/6``：先记录余数 1，``10=1*6+4`` 生成非循环数字 1；再记录余数 4，
  ``40=6*6+4`` 生成数字 6 后回到余数 4，因此得到 ``"0.1(6)"``，而不是 ``"0.(16)"``。
* ``-50/8``：符号单独记录，正数幅度先得到整数部分 6 和余数 2；随后
  ``20=2*8+4``、``40=5*8+0``，余数归零，有限结果为 ``"-6.25"``。
...
* **Rust**：``i32`` 先转 ``i64``；记录的是 UTF-8 字节位置，但输出全为 ASCII，
  ``String.insert`` 边界合法。
* **Go**：算术使用 ``int64``；扩展字节切片后再执行重叠 ``copy``，源范围使用插入前长度，不覆盖后缀。
* **TypeScript**：只用普通算术和 ``Math.trunc``；没有触发 32 位位运算转换，所有整数精确。
* **Julia**：``Int64`` 关闭最小整数边界；字节向量一基位置加一映射正确，
  ``String(bytes)`` 接收合法 ASCII。
...
* 十进制长除法的余数状态转移和精确前缀不变量；
* 有限余数集合保证“终止或进入循环”二分；
* 首次重复余数给出函数图的环入口和括号位置；
* 输出敏感复杂度同时核算状态表、字符串构造与一次括号插入；
* C 动态字符串和开放寻址余数表的完整失败事务。

强化
~~~~

* 复用 0029 与 0050 已讲解的“先扩宽再处理最小有符号整数”，本题把它用于分子、分母取绝对值；
* 继续区分核心算法状态、语言适配成本、返回载荷和哈希最坏风险。

关联题目
~~~~~~~~

* `0050. Pow(x, n) <../0001-0100/0050-powx-n.rst>`_：两题都必须在取负前扩宽
  32 位最小整数；
* `0141. Linked List Cycle <0141-linked-list-cycle.rst>`_：两题都识别有限状态重复，
  但本题还需要保存首次状态对应的输出位置。
...
#. **时间和空间为什么不能写成 ``O(1)``？**
   ...
   算法必须生成长度 ``L`` 的返回字符串，并保存最多 ``L`` 个不同余数位置，
   所以输出与辅助空间都随 ``L`` 增长。
