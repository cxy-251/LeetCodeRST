...
C、C++、Java、Rust、Go、TypeScript、C#、Julia 与 R 实现都使用这一虚拟读取，
避免每次比较分配两个临时拼接串。Python 也采用同样比较函数并通过
``cmp_to_key`` 适配排序接口。
...
有限比较 ``ab`` 与 ``ba`` 和这个无限周期比较给出同一顺序：两个周期串的比较模式由
``a`` 与 ``b`` 的字符周期共同决定；若在 ``ab`` 与 ``ba`` 中出现首个差异，它也是无限串的首个差异；
若 ``ab=ba``，两串由同一个基本周期重复，因而 ``a^ω=b^ω``。
...
* C 比较器不分配临时串，成功返回的 ``char*`` 归调用者释放；所有中间字符串在返回前释放；
* Python ``cmp_to_key`` 创建包装对象；Java、C#、Rust、Go、TypeScript 与 Julia 使用各自排序容器；
* R 采用自写底向上归并排序，因为基础 ``sort`` 不接受本题比较器；工作缓冲为 ``O(n)``，
  字符串字节表示为 ``O(D)``。
