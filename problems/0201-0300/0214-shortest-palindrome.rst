...
:访问状态: Available
:教学重点: 把最短构造转化为最长回文前缀，并用 KMP 最终 border 在线性时间求出该前缀

精确契约
--------

给定字符串 ``s``，只能在它的前面添加字符。返回通过这种操作能够得到的最短回文字符串。
...
原字符串必须完整保留为结果的后缀，不能改变字符顺序，也不能在中间或末尾添加字符。空字符串和已经是回文的字符串不需要添加任何内容。

官方字符域是 ``a..z``，因此本文可以安全地使用不在输入域中的 ``#`` 作为分隔符，并按字节或普通字符处理。若输入扩展为任意 Unicode 文本，需要重新定义“字符”单位和分隔符表示。
...
任何非空字符串的首字符都是长度为 1 的回文前缀，所以最坏只需添加后 ``n-1`` 个字符的反转。
