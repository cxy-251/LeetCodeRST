...
:题号: 0207
:难度: Medium
:主题: 有向图、拓扑排序、入度、Kahn 算法
:原题: `LeetCode 0207 <https://leetcode.com/problems/course-schedule/>`_
:访问状态: Available
:教学重点: 先修边方向、剩余入度不变量、环与拓扑序等价、邻接表资源

精确契约
--------

共有 ``numCourses`` 门课程，编号为 ``0`` 到 ``numCourses-1``。每个先修对
``[course, prerequisite]`` 表示：学习 ``course`` 之前必须先完成 ``prerequisite``。

判断是否存在一种课程完成顺序，使全部课程都满足所有先修要求。存在时返回 ``true``，否则返回 ``false``。

本文采用以下契约：

* ``numCourses`` 为非负整数；官方输入至少有一门课程，``0`` 门课程作为扩展边界返回 ``true``；
* 每个先修对包含两个合法课程编号；
* 一条先修对建立有向边 ``prerequisite -> course``；
* 输入中可能有自环、重复边、孤立课程和多个互不连接的分量；
* 重复边按输入逐条保留：邻接表出现几次，目标课程入度就增加几次；
* 函数只读 ``prerequisites``，不要求输出实际课程顺序；
* 返回 ``false`` 既可能表示图中有环；C 适配器若资源分配失败也只能返回 ``false``，该接口限制会单独说明。

建边方向
--------

先修对 ``[1,0]`` 的含义是“先学 0，再学 1”，所以边必须是：

.. code-block:: text

   0 -> 1

``indegree[course]`` 表示这门课程当前仍有多少条来自未处理课程的先修边。若错误建立
``course -> prerequisite``，虽然“图是否有环”在整张图反向后仍保持不变，但代码中的入度、队列含义和输出顺序解释会全部颠倒；本章始终使用
``prerequisite -> course``。
...
课程 0 先进入队列。处理 0 后，课程 1 的入度变为 0，再处理 1，最终处理数为 2，返回 ``true``。
...
没有课程能进入初始队列，处理数为 0，返回 ``false``。

自环
~~~~

``[[0,0]]`` 建立 ``0 -> 0``。课程 0 的入度为 1，且只有处理课程 0 才能删除这条边；它永远无法先进入队列，因此返回 ``false``。
...
对应先修对 ``[[1,0],[2,0],[3,1],[3,2]]``。一种合法顺序是 ``0,1,2,3``，也可以交换 1 与 2。

孤立课程与多个分量
~~~~~~~~~~~~~~~~~~

没有入边的孤立课程初始入度为 0，应与其他分量的零入度课程一起入队。算法不要求图连通。

重复边
~~~~~~

若输入包含两次 ``[1,0]``，则邻接表中保留两条 ``0 -> 1``，``indegree[1]=2``。处理课程 0 时也会遍历两条边并递减两次，使入度恰好归零。不能只在邻接表去重，却仍把入度增加两次。
