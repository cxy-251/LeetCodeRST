...
:题号: 0211
:难度: Medium
:主题: Trie、通配符、深度优先搜索、可变对象
:原题: `LeetCode 0211 <https://leetcode.com/problems/design-add-and-search-words-data-structure/>`_
:访问状态: Available
:教学重点: 点号单字符语义、分支搜索、终止标记、真实访问规模与对象生命周期

精确契约
--------

设计一个 ``WordDictionary`` 对象，按调用顺序支持：

``addWord(word)``
   把一个由小写英文字母组成的完整单词加入字典。重复加入不会改变查询结果。

``search(pattern)``
   判断是否存在此前加入的某个完整单词与 ``pattern`` 匹配。模式中的小写字母必须匹配相同字母，
   点号 ``.`` 匹配恰好一个任意小写字母。

关键语义：

* 点号不能匹配零个字符，也不能匹配任意长度字符串；
* 模式长度必须与被匹配单词长度相同；
* 搜索消费完整个模式后，当前 Trie 节点必须带有 ``is_word=true``；
* 只存在路径或只匹配到某个单词前缀仍不足以返回真；
* 字典对象有持久状态，后续搜索能看到此前所有成功加入的单词；
* 官方字符域为小写英文字母和搜索模式中的点号，固定 26 子链接实现不扩展到任意 Unicode；
* 官方参数非空。本文扩展约定 ``addWord("")`` 标记根节点，``search("")`` 返回根节点终止标记。
...
``is_word``
   从根到当前节点的完整字符序列是否曾被成功加入。

``addWord`` 与普通 Trie 插入相同：逐字符复用或创建路径，最后只标记终点。新增难点全部集中在搜索阶段：
字母只沿一个确定分支前进，点号必须尝试当前节点的所有非空子链接。
...
``.ad`` 的第一个点号可以选择 ``b``、``d`` 或 ``m``；只要任一分支的剩余 ``ad`` 匹配完整单词就返回真。
