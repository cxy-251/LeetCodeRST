...
:访问状态: Available
:教学重点: 用“排除首屋或排除尾屋”消除环形冲突，再复用线性不相邻选择递推

精确契约
--------

给定非负整数数组 ``nums``，相邻房屋不能同时选择，并且第 0 间与第 ``n-1`` 间也相邻。返回合法选择能够得到的最大金额。
...
最大可能总金额不超过 ``100000``，普通 32 位有符号整数足够。本文实现不修改输入。

单元素必须单独处理：只有一间房时不存在两间不同房屋之间的冲突，答案就是 ``nums[0]``。
