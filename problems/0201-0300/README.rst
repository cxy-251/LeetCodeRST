...
* ``0203``：虚拟头连接不变量、连续删除、C 节点释放假设、Rust 所有权槽位和平台节点复用；
* ``0204``：严格小于 ``n`` 的质数计数、埃氏筛不变量、``p*p`` 起点证明、溢出安全和筛数组成本；
* ``0205``：字符双向映射、函数一致性、目标唯一性、双射证明和跨语言字符单位；
* ``0206``：``previous/current/next`` 原地反转、节点守恒、无环证明、平台节点复用和跨语言所有权；
...
新题生成入口仍为 ``0219``，但 ``0219–0220`` 当前暂停。项目正在升序审查既有题解；
当前审查入口、风险和黄金样本以 ``../../state/PROGRESS.toml`` 与
``../../state/BATCH_CONTRACT.toml`` 为准，历史审查完成后再恢复本目录的新题生成。
