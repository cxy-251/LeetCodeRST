...
本目录按题号顺序保存 0201 至 0300 的算法教程。当前已完成 0201 至 0216，下一入口为 0217。
...
* ``0216``：严格递增组合回溯、槽位与剩余和守恒、最小/最大可达和剪枝、结果快照和 C 二维所有权。
...
下一批独立生成 ``0217`` Contains Duplicate。重点处理哈希集合正确性、平均与最坏复杂度、
C 开放寻址的独立占用标记与容量计算，以及 R 无歧义整数键；具体合同见
