...
本目录按题号顺序保存 0201 至 0300 的算法教程。当前已完成 0201 至 0217，下一入口为 0218。
...
* ``0216``：严格递增组合回溯、数量与总和守恒、可达和剪枝、结果快照和 C 二维结果所有权；
* ``0217``：已见集合不变量、哈希平均与最坏复杂度、C 开放寻址和 R 无歧义整数键。
...
下一批独立生成 ``0218 The Skyline Problem``。重点处理同坐标事件分组、活动建筑最大高度、
过期堆元素惰性清理、关键点规范化、完整正确性证明和十语言堆/二维输出资源；具体合同见
