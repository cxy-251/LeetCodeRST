...
* ``0086``：稳定双链分区、逐节点断开、节点身份守恒与分区内顺序证明正确；
* ``0087``：三维区间状态、交换与不交换转移、频次剪枝及真假缓存完整；
* ``0088``：逆向写入和未读前缀保护正确；Julia 修改可见性说明已登记事实更正；
* ``0089``：官方输入域内 Gray 公式、可逆覆盖与首尾闭环正确；``n = 0`` 扩展需显式退化约定；
* ``0090``：排序聚类、同层规范代表、深层重复副本与独立结果快照正确。

本批新增两项事实更正并写入 ``state/REVIEW_INDEX.toml``，历史题目正文保持原样。
