...
* ``0061``：取模切分、临时成环和断环证明完整；Rust 使用可证明等价的线性所有权变换；
* ``0062``：一维路径计数正确；最终上界约束中间状态需要补充前缀路径到终点路径的单射；
* ``0063``：障碍归零和饱和计数正确；障碍会破坏最终答案对中间状态的支配；
* ``0064``：边界初始化和最优子结构完整；数值上界应按单条路径长度推导；
* ``0065``：数字语法扫描正确；Julia ``codeunits`` 是轻量包装视图，不是 ``O(n)`` 物化副本。

``0065`` 的复杂度事实更正已登记到 ``state/REVIEW_INDEX.toml``，本轮不回改历史题目正文。
