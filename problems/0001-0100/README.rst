...
* ``0071``：组件栈和 C 回滚缓冲正确；R 动态字符栈最坏产生平方级累计复制；
* ``0072``：一行 DP 转移正确；C++、Julia、R 适配器会产生 ``O(m+n)`` 输入物化空间；
* ``0073``：首行首列标记与四阶段写入正确；R 返回副本空间应与核心 ``O(1)`` 状态区分；
* ``0074``：虚拟扁平二分正确；整体有序、规则矩阵和元素总数容量是必要前提；
* ``0075``：四段荷兰国旗不变量正确；默认分支依赖输入严格限定为 ``0``、``1``、``2``。

``0071`` 与 ``0072`` 的复杂度事实更正已登记到 ``state/REVIEW_INDEX.toml``，本轮不回改历史正文。
