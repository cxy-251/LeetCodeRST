...
* ``0081``：重复端点消歧和有序半区二分正确；最坏复杂度因信息退化降为 ``O(n)``；
* ``0082``：哨兵前驱完整删除重复值段；断链与节点回收责任按语言区分；
* ``0083``：连续绕过同值后继并保留每段首节点；与 0082 的值段契约不可混用；
* ``0084``：单调栈左右边界、等高策略和局部零哨兵证明完整；
* ``0085``：逐行柱高降维与唯一底边映射正确；不同语言的栈复用和分配行为需分层。

本批没有新增事实更正，历史题目正文保持原样。
