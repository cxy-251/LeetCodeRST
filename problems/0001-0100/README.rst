...
``0001`` 至 ``0051`` 的首轮教程已经完成。``0001`` 至 ``0050`` 的审查规则正在应用于
``0051`` 至 ``0100`` 的新题。

下一内容批次
------------

下一批从 ``0052`` 开始，按难度预算连续处理：

* ``0052. N-Queens II``：Hard；
* ``0053. Maximum Subarray``：Medium。

两题总分为 6，符合单批预算，并且只有一道 Hard。

0051 学习重点
-------------

* 按行回溯把行冲突从状态中消除；
* 用列、``row-col`` 和 ``row+col`` 三组占用状态常数时间判断冲突；
* 证明合法棋盘与按行列选择序列一一对应；
* 分开搜索节点成本与每个棋盘 ``n²`` 字符的输出成本；
* C 多层结果结构使用事务式扩容和完整失败清理。
