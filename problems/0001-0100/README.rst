...
``0001–0050`` 首轮教程已完成，现进入逐文件全量审查。审查完成并由用户明确解除检查点前，
禁止生成 ``0051``。

阅读顺序
--------

建议按题号连续阅读。审查时逐题核对题意、主算法、正确性证明、复杂度、十语言实现、语言边界、
内存所有权、索引语义、自检答案和知识账本。

0050 审查检查点
---------------

发现确定性缺陷时使用独立 ``fix:`` PR，修复不得新增题目或推进题号。深夜任务读取到
``next_problem >= 51`` 时只报告等待审查，不写仓库。

关联题目
--------

单题只保留 1 至 3 个最强关联，最多 5 个。没有强关联时省略该节。完整同类题目以后
由专题索引维护，详细规则见 ``../../docs/RELATED_PROBLEMS_POLICY.rst``。
