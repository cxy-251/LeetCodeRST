...
``0001`` 至 ``0053`` 的首轮教程已经完成。``0001`` 至 ``0050`` 的审查规则正在应用于
``0051`` 至 ``0100`` 的新题。

下一内容批次
------------

下一批从 ``0054`` 开始，连续处理三道 Medium：
...
* 用当前行位掩码表示列与两类对角线攻击状态；
* 通过左移、右移把对角线约束传递到下一行；
* 计数型回溯只累计叶子，不构造棋盘输出；
* Kadane 状态表示必须以当前位置结尾的最大连续子数组和；
* 全负数组从首元素初始化，禁止把空子数组和 0 混入候选；
* 按语言真实语义区分 Python 切片复制与 Go 切片描述符。
