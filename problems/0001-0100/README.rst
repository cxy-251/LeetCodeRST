...
#. `0052. N-Queens II <0052-n-queens-ii.rst>`_
#. `0053. Maximum Subarray <0053-maximum-subarray.rst>`_
#. `0054. Spiral Matrix <0054-spiral-matrix.rst>`_
#. `0055. Jump Game <0055-jump-game.rst>`_
#. `0056. Merge Intervals <0056-merge-intervals.rst>`_
#. `0057. Insert Interval <0057-insert-interval.rst>`_
#. `0058. Length of Last Word <0058-length-of-last-word.rst>`_
#. `0059. Spiral Matrix II <0059-spiral-matrix-ii.rst>`_
#. `0060. Permutation Sequence <0060-permutation-sequence.rst>`_
#. `0061. Rotate List <0061-rotate-list.rst>`_
#. `0062. Unique Paths <0062-unique-paths.rst>`_
#. `0063. Unique Paths II <0063-unique-paths-ii.rst>`_
#. `0064. Minimum Path Sum <0064-minimum-path-sum.rst>`_
#. `0065. Valid Number <0065-valid-number.rst>`_
#. `0066. Plus One <0066-plus-one.rst>`_
#. `0067. Add Binary <0067-add-binary.rst>`_
#. `0068. Text Justification <0068-text-justification.rst>`_
#. `0069. Sqrt(x) <0069-sqrtx.rst>`_
#. `0070. Climbing Stairs <0070-climbing-stairs.rst>`_
#. `0071. Simplify Path <0071-simplify-path.rst>`_
#. `0072. Edit Distance <0072-edit-distance.rst>`_
...
#. `0087. Scramble String <0087-scramble-string.rst>`_
#. `0088. Merge Sorted Array <0088-merge-sorted-array.rst>`_
...
``0001`` 至 ``0100`` 的首轮教程已经完成。``0051`` 至 ``0100`` 的第二轮逐题审查也已全部完成，
历史题目正文保持原样。

本批审查结论
------------

* ``0096``：节点数 Catalan 状态、空树单位元、左右计数乘法和宽中间值正确；
* ``0097``：双前缀状态、一行 DP 左到右覆盖和短串列压缩正确；C++、Julia、R 的适配器物化空间已登记更正；
* ``0098``：中序严格递增充要条件、跨层约束和独立前驱存在标志正确；
* ``0099``：第一次下降左端、最后一次下降右端和延迟交换值正确，树拓扑保持不变；
* ``0100``：成对递归、空节点结构基例、左右同向比较和短路边界正确。

本批新增一项事实更正并写入 ``state/REVIEW_INDEX.toml``，历史题目正文保持原样。

下一动作
--------

归并 ``0051`` 至 ``0100`` 第二轮审查中的稳定规则、事实更正和验证限制，形成下一阶段完整前向规则。
规则归并完成前，不启动 ``0101`` 之后的题目生成。
