...
``0001`` 至 ``0061`` 的首轮教程已经完成。``0001`` 至 ``0050`` 的审查规则正在应用于
``0051`` 至 ``0100`` 的新题。
...
* 把一基排名 ``k`` 转换为零基秩，再按阶乘块选择下一位数字；
* 证明每个候选分支恰好包含 ``(remaining - 1)!`` 个排列；
* 计入候选动态数组中间删除造成的 ``O(n²)`` 总成本；
* 把右旋 ``k`` 次化为 ``k % length`` 后的单次切分与拼接；
* 临时成环后在唯一切点断开，保持节点身份、相对顺序和最终无环；
* Rust 使用线性断开再拼接，表达与成环方案相同的节点顺序。
