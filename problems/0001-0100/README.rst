...
``0001`` 至 ``0067`` 的首轮教程已经完成。``0001`` 至 ``0050`` 的审查规则正在应用于
``0051`` 至 ``0100`` 的新题。
...
三题难度总分为 6，符合单批预算，并且只有一道 Hard。

0065–0067 学习重点
------------------

* 用有限布尔状态验证符号、小数点、指数和数字见证，而不构造浮点数；
* 区分“底数已经有数字”与“指数之后已经有数字”，防止接受未完成指数；
* 从右向左扫描连续 ``9`` 后缀，遇到首个非 ``9`` 数字时终止十进制进位；
* 证明全 ``9`` 是 ``Plus One`` 唯一需要扩位的输入；
* 让不同长度二进制字符串按最低位对齐，并维护唯一进位；
* 使用反向追加再反转或从固定缓冲区末尾写入，避免循环前置导致平方级复制。
