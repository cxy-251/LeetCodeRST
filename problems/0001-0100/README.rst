...
``0001`` 至 ``0073`` 的首轮教程已经完成。``0001`` 至 ``0050`` 的审查规则正在应用于
``0051`` 至 ``0100`` 的新题。
...
两题难度总分为 4；``0076`` 为 Hard，加入后会超过单批预算。

0071–0073 学习重点
------------------

* 把绝对路径切分为组件，用栈消解 ``.``、``..`` 和重复斜杠；
* 只把完整组件 ``.`` 与 ``..`` 解释为特殊标记，其他含点名称原样保留；
* 用前缀动态规划表达插入、删除和替换，并保存一行覆盖所需的对角旧值；
* 复用矩阵首行与首列保存行列标记，用两个布尔值保留边界原始零；
* 按保存边界、写入标记、应用内部、恢复边界的顺序避免标记污染。
