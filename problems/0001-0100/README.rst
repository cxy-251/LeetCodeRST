...
``0001`` 至 ``0050`` 的首轮教程已经完成。当前正在逐题完整读取这些 RST，提炼
``0051`` 至 ``0100`` 的前向质量规则。

审查约束：

* 审查依据是题目正文和十语言代码，不是 PR 描述；
* 每道题都需要完整读取；
* 只更新审查记录和公共前向规则；
* 不修改 ``0001`` 至 ``0050`` 的题目文件；
* 审查完成前暂不生成 ``0051``。

总进度见 ``../../state/REVIEW_INDEX.toml`` 与 ``../../state/PROGRESS.toml``。
前向规则见 ``../../docs/FORWARD_RULES_0051_0100.rst``。

下一审查批次
------------

当前已审查 ``0001`` 至 ``0035``。下一批完整读取 ``0036`` 至 ``0040``。

本轮新增的前向检查包括：无边界扫描的见证元素、哨兵栈非空不变量、必要修复直接并入主代码、
索引窄化覆盖末尾边界 ``n``、原地排列邻接证明、二分返回语义、单调谓词命名，以及 C 分配结果在首次写入前检查。

关联题目
--------

单题只保留 1 至 3 个最强关联，最多 5 个。没有强关联时省略该节。完整规则见
