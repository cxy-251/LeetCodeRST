...
``0001`` 至 ``0064`` 的首轮教程已经完成。``0001`` 至 ``0050`` 的审查规则正在应用于
``0051`` 至 ``0100`` 的新题。
...
* 按最后一步来自上方或左方，把网格路径问题写成二维动态规划；
* 使用一维数组同时保留上方旧状态与左侧新状态，并证明从左向右覆盖合法；
* 在障碍位置把路径状态归零，准确阻断向右和向下传播；
* 对障碍造成的巨大失效前缀计数使用 ``2_000_000_001`` 饱和，保持最终答案精确；
* 把路径计数转为最小路径和时，证明最优前缀替换和第一行、第一列初始化；
* 由最大路径长度和单元格上界推导最小路径和不超过 ``79800``。
