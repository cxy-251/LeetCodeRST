...
``0001`` 至 ``0050`` 的首轮教程已经完成。当前逐题读取这些 RST，从正文和十语言代码中提炼
``0051`` 至 ``0100`` 的前向质量规则。
...
当前已审查 ``0001`` 至 ``0045``。下一批完整读取 ``0046`` 至 ``0050``。

本轮新增的前向检查包括：值派生下标的短路守卫、永久槽位与重复值终止、聚合结果宽度、数字字符串
规范表示、后处理见证、序列化管线、受控回退单调进度、输入规范化空间、区间 BFS 层边界和可达性进展契约。

关联题目
--------

单题保留 1 至 3 个最强关联，最多 5 个。没有强关联时省略该节。完整规则见
